...
export const filterProducts = async (req, res) => {
  try {
    ...
    // Build query object
    const query = { status: "approved", is_deleted: false };
...
export const getFilterOptions = async (req, res) => {
  try {
    ...
    const priceStats = await Product.aggregate([
      { $match: { status: 'approved', is_deleted: false } },
      {
        $group: {
          _id: null,
    ...
    res.json({
      ...
      priceRanges: [
        { label: 'All Price', min: 0, max: null, count: await Product.countDocuments({ status: 'approved', is_deleted: false }) },
